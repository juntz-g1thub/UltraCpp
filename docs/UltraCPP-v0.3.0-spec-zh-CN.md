...
> **日期**：2026-08-07
>
> **语言权威性**：本中文版为**权威版本**；英文版 [`UltraCPP-v0.3.0-spec-en.md`](./UltraCPP-v0.3.0-spec-en.md) 为对照译本，如有歧义以本文为准。
...
### 11.6 `alloc` / `free` 配对约定 *(0.3.0 沿用 0.2.0 · D-6)*
...
shared atomic<int> flag;        // 显式 atomic 包装（atomic<T> 是 stdlib 类型，见 §11.5.1）
```
...
void worker_atomic() {
    int v = flag.load();    // 硬件原子加载
    flag.store(v + 1);      // 硬件原子存储
}
```
