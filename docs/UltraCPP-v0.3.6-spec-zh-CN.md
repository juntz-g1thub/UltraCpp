...
> **版本**：0.3.6
>
> **上一版本**：0.3.5 — [`UltraCPP-v0.3.5-spec-zh-CN.md`](./UltraCPP-v0.3.5-spec-zh-CN.md)
>
> **状态**：草稿 (draft)
>
> **日期**：2026-08-28
>
> 同版本英文译本：[English](./UltraCPP-v0.3.6-spec-en.md)
...
**注 1**：main 函数返回 `int` 类型已在 0.3.5 末态正常工作（per m0_02/03/08 翻 PASS 推导）。返回码语义遵循 C 标准：0 = 成功；非 0 = 失败。
...
// === [0.3.3] 重分类为 builtin 函数（最终形态）===
// mod / unmod / move / clone 不再是运算符，而是由 codegen 分发的 builtin 函数调用。
// alloc / free 保持 builtin 函数调用形式（0.3.2 起即为函数调用形式）。
// 见 §7.0.3 重分类为 builtin 函数 与 §11.0.1 builtin 语言原语。
builtin_function_call ::= 'alloc' '(' type ')'
                       |  'free' '(' expression ')'
                       |  'move' '(' expression ')'
                       |  'clone' '(' expression ')'
                       |  'mod' '(' expression ')'                       // [0.3.3] 获取 mod-right（0.3.0/0.3.1/0.3.2 为运算符）
                       |  'unmod' '(' expression ')'                     // [0.3.3] 释放 mod-right（0.3.0/0.3.1/0.3.2 为运算符）

// === [0.3.3] Intrinsic 调用（codegen 特化；不进 BUILTIN_SIGS[]）===
// 见 runtime-architecture §5.2 与 §11.0.1（intrinsics 共 4 项）。
intrinsic_call     ::= 'null' '(' ')'
                    |  'is_null' '(' expression ')'
                    |  'sizeof' '(' type ')'
                    |  'alignof' '(' type ')'

// === [0.3.3] 预编译指令（C 风格，`#`-prefixed）===
// 见 §10.2 预编译宏机制。
...
// === [0.3.3] 限定（namespace）调用 ===
// 见 §10.4 `sys::` 系统调用 namespace。
...
**Resolution**: ✅ Closed in 0.3.5（per HANDOFF.md 0.3.5 实施记录：commit 14a-errata spec 第 1 组变更；commit 14b fn-ptr；commit 14c #include 语义；commit 14d multi-TU #import；commit 14e M1 关键字 lexer；commit 14f compound_assign；commit 14g UC_TYPE_MUTABLE_POINTER 死代码清理）
...
**Resolution**: ✅ Closed in 0.3.6（方案 A，per S6 决策 + D10' + D12）：9 项 `@`-directive 改 `#`-prefix（§10.2.1 表 8 行 + §10.2.2 1 行 + §10.2.3 表 8 行 + §10.4.3 2 行 + §10.4 交叉引用 1 行 + §10.5.1 错误码表 1 行 + §11.0.2 主表 1 行 + §12.1 EBNF 2 行 + §14 §S10 entry 本身）；原 0.3.5 spec §10.2 第 (4) 条 stale rationale 删除；`@`-prefix 彻底删除（不保留 deprecated alias）；lexer 端 `scan_at_directive` 删除属 0.3.7+ 范围。
...
| 0.3.6 | 2026-08-28 | §10.2 preprocessor prefix 统一到 `#`（方案 A，per S6）；§10.4 sys:: 注 1 段；§11.0.2 删 lib/io.uc 注 2；§14 §S6-§S9 刷新 + §S10 新增；§6.5 main return 注 1 |
