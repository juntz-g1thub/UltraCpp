# UltraCPP 0.3.6 Language Specification

> **Version**: 0.3.6
>
> **Previous version**: 0.3.5 — [`UltraCPP-v0.3.5-spec-en.md`](./UltraCPP-v0.3.5-spec-en.md)
>
> **Status**: draft
>
> **Date**: 2026-08-28
>
> Same-version Chinese translation: [简体中文](./UltraCPP-v0.3.6-spec-zh-CN.md)
...
**Note 1**: The `int` return type of `main` works correctly as of 0.3.5 (derived from m0_02/03/08 flipping to PASS). Return code semantics follow the C standard: 0 = success; non-zero = failure.
...
UltraCPP adopts **C-style preprocessor macro syntax** (`#`-prefixed), not Rust's `#[cfg(...)]` style. Rationale: compatibility with the C ecosystem toolchain; seamless integration with the existing C implementation in `src-c/`; gentle learning curve; visual alignment with `#include` / `#import` / `#modlaw`.

Note: `@`-prefixed forms were used from 0.3.3 to 0.3.5; starting 0.3.6 they are unified to `#`-prefix (per §10.2 unification, S6 decision).
...
|---|---|---|---|
| `lib/print.uc` | `uc_print_str` (full implementation: `sys::strlen` + `sys::write`) / `uc_print_num` (full itoa: negative + 16-byte buffer + reverse + `sys::write`) / `uc_print_float` (STUB: writes `<float>` string; commit 14h (0.3.5) full implementation as simple ftoa (2 decimal places)) | 10a | ~50–80 |
...
**Resolution**: ✅ Closed in 0.3.5 (per HANDOFF.md 0.3.5 implementation record: commit 14a-errata spec change group 1; commit 14b fn-ptr; commit 14c #include semantic; commit 14d multi-TU #import; commit 14e M1 keyword lexer; commit 14f compound_assign; commit 14g UC_TYPE_MUTABLE_POINTER dead code cleanup)

**References**: see §10.1 `extern "C"` Block; see runtime-architecture §3.1.

### §S7 CJK identifier support *(Permanently NO-OP, resolved 2026-08-21 per user decision)*

Non-ASCII identifiers (including CJK / UTF-8) are permanently out of scope. Spec §2.4 explicitly declares ASCII-only (per 0.3.5 revision).

**Resolution**: ✅ Closed in 0.3.5 (per HANDOFF.md 0.3.5 implementation record: commit 14a-errata spec change group 1; commit 14b fn-ptr; commit 14c #include semantic; commit 14d multi-TU #import; commit 14e M1 keyword lexer; commit 14f compound_assign; commit 14g UC_TYPE_MUTABLE_POINTER dead code cleanup)

**History**: the 0.3.0 priority doc §9.x listed S7 as deferred (P1-5 scope); 0.3.4 commit `02d7170` closed the m0 baseline CJK FAIL via `expects_compiler_error` runner marker + m0_50 .uc marker (the compiler's lexer already correctly outputs `unknown character`; the old runner logic uniformly treating compile_failed as FAIL was a misjudgment). 0.3.5 spec resolution: S7 will never enter any future version; spec §2.4 explicit ASCII-only is the sole anchor.

**m0 test impact**: m0_50 marked PASS (commit `02d7170`). This resolution has no new impact on the m0 baseline (already PASS).

**References**: see spec §2.4 Identifier Rules (0.3.5 explicit addition); see `.dev/drafts/0.3.0-m0-priority.md` §9.x S7.
...
### §S10 Preprocessor prefix unified to `#` (added in 0.3.6, S6 decision)

**Resolution**: ✅ Closed in 0.3.6 (Plan A, per S6 decision + D10' + D12): 9 `@`-directives changed to `#`-prefix (§10.2.1 table 8 rows + §10.2.2 1 line + §10.2.3 table 8 rows + §10.4.3 2 lines + §10.4 cross-reference 1 line + §10.5.1 error code table 1 line + §11.0.2 main table 1 line + §12.1 EBNF 2 lines + §14 §S10 entry itself); original 0.3.5 spec §10.2 rationale item (4) deleted as stale; `@`-prefix completely removed (no deprecated alias retained); lexer-side `scan_at_directive` removal belongs to 0.3.7+ scope.
...
| 0.3.6 | 2026-08-28 | §10.2 preprocessor prefix unified to `#` (Plan A, per S6); §10.4 sys:: note 1; §11.0.2 remove lib/io.uc note 2; §14 §S6-§S9 refresh + §S10 new entry; §6.5 main return note 1 |
