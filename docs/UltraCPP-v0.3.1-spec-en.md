# UltraCPP 0.3.1 Language Specification

> **Version**: 0.3.1
>
> **Previous version**: 0.3.0 — [`UltraCPP-v0.3.0-spec-en.md`](./UltraCPP-v0.3.0-spec-en.md)
>
> **Status**: draft
>
> **Date**: 2026-08-12
>
> Same-version Chinese translation: [简体中文](./UltraCPP-v0.3.1-spec-zh-CN.md)
...
|------|------|------|
| Rule 1 | §3.2 | Owning types are unrestricted between heap/stack; storage location is inferred from the initialization expression. |
| Rule 2A | §3.3 | Reference `&` forbids owning; compile-time null-ref reports. |
| Rule 2B | §3.2 | `T*` decides owning vs. non-owning by its initialization; assignment does not transfer ownership, but grants the modification right. |
| Q1 | §3.7 | `unique T` ≡ `T*`, the `unique` keyword may be omitted. |
| Q2 | §3.3, §12.1 | The right side of a reference declaration is an lvalue; the right side of a pointer declaration is address-of. |
...
| **Q3 inverted** *(revised in 0.3.0)* | §3.3, §7.8, §7.9 (removed), §12.1, §12.2, §2.3, §2.6 | **Remove the `T&mut` type**: references are unified as `T&`; exclusivity is now controlled by `#modlaw exclusive` + `mod()` requests, no longer distinguished at the type level. |
| **Rule 23** | §9.9 | Add the `#modlaw <perm> <scope>` directive (6 legal combinations). |
| **Rule 24** | §4.9, §4.10 | Add `mod(ref_expr)` and `unmod(ref_expr)` expressions. |
...
| **S2 (revised in 0.3.1)** | §4.1, §4.6, §4.13 (new), §7.8, §12.1, §12.3 | **lvalue / rvalue concepts made explicit**: add the complete §4.13 chapter (definitions + classification table + assignment-context constraint + codegen implementation constraint + cross-references); the §4.1 precedence table adds level 2.5 for unary ops (` * ` ` & ` ` + ` ` - ` ` ! ` ` ~ ` `mod` `unmod` prefix `++` `--`); all 11 rows of the §4.6 table uniformly add the "LHS must be an lvalue (§4.13.2)" constraint; §7.8 creation rule 1 references §4.13.2 and adds legal/illegal examples; §12.1 EBNF adds `lvalue` / `rvalue` rules and the `assignment_expression` LHS annotation; §12.3 appendix precedence table adds level 2.5 to match. **Fix the m0_42 deref-assign bug** (`*view = payload`). No new syntax, no semantic change, backward compatible. |
...
> **[0.3.1 revision]** 0.3.1 explicitly defines the lvalue / rvalue concepts (§4.13); the "operand must be an lvalue" wording in this section references the
> §4.13.2 expression classification table.
...
1. The operand must be an lvalue (see the §4.13.2 expression classification table).
...
### 11.6 `alloc` / `free` Pairing Convention *(carried over from 0.2.0 in 0.3.0 · D-6)*
...
identifier         ::= letter (letter | digit)*
letter             ::= 'a'..'z' | 'A'..'Z' | '_'                      // e.g. '__thread' starts with '_' '_' 't' ...
digit              ::= '0'..'9'
```
...
### 13.3 Cross-Thread Modification Right (mutex / atomic)
