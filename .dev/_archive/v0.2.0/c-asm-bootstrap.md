...
> **作者**: UltraCPP Team
> **状态**: Phase 1 + 1.1 + 2 + 3 + 4 ✅ 完成。Phase 5（asm-Lexer）放弃（详见本计划 §7 已更新决策）。后续路线图见 [`../../bootstrap/PLAN.md`](../../bootstrap/PLAN.md)。
>
> **本文档性质**：历史规划档案（描述 Phase 1-4 实施过程与决策）。当前活跃路线图请看 `bootstrap/PLAN.md`（2026-08-06 启动 bootstrap 时新建）。
...
## 12. 后续（2026-08-06 bootstrap 启动）

本文档记述到 Phase 4。Phase 5（asm-Lexer）调试成本过高已放弃，asm 路径整体删除（`src-asm/`）。Bootstrap 路径（用 C 端做底层，用 UltraCPP 写自己）已启动：

- 路线图：[`../../bootstrap/PLAN.md`](../../bootstrap/PLAN.md)
- 目标基线：[`../../bootstrap/baseline/`](../../bootstrap/baseline/)（test_t1 的 IR/asm，C/Rust 端口字节级一致）
- 新编译器源码目录：[`../../src-uc/`](../../src-uc/)（待编写）
- 详细决策：见 [`../../HANDOFF.md`](../../HANDOFF.md) §五 §六
