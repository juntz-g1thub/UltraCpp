...
Current progress: M0 P0-1 (const trio m0_22/45/46) + P0-2 (alloc/free/move/unique trio m0_34/35/36) + **P0-3 (++/-- + ternary m0_47/48)** + **P0-4 (void + local/global m0_17/21)** all complete + **P1-2 (m0_10 else-if chain)** + **P1-3 (for + break/continue m0_13/14/15)** + **P1-4 partial (m0_04 comparison)** all complete; baseline **23/48 PASS** + **25/48 FAIL** (m0_35 full e2e exit=0; m0_34/36 parser+IR OK, llc codegen gap `*p = X` deref-assign 是 known issue, P3-5 时回看; m0_47 exit=16 full PASS + m0_48 exit=20 **effective PASS** — runner 误判因 expected-code extraction 默认 0; P0-3 期间 5 codegen fixes (cf51f6f / f1f4214 / 3abe96b) 同时铺平 control flow, m0_09 等 control flow 测试也开始水落石出 PASS; **m0_17 just flipped from compile_failed → ✓ exit=0 with commit `c052d2c` (void 函数 return path 修复)**; m0_21 早已 PASS — P0-4 净增 1 通过; **m0_10 just flipped** (commit `09b4df5`) — **不是 codegen bug**, 是 test runner awk 不解析函数调用式 return, 加 trailing `// 255` 注释修复; **m0_04 just flipped** (commits `5abb262` codegen i1→i32 zext + `ef3352c` test trailing `// 3`); **m0_13 + m0_14 + m0_15 just flipped** (commits `afae9c9` UC_STMT_FOR/BREAK/CONTINUE + loop scope 栈 + `c8492ef` UC_BIN_MOD + UC_EXPR_LITERAL last_expr_type per lit kind; m0_14 was always FAIL (for-loop codegen was a stub until afae9c9; P1-3 codegen change exposed latent literal-type leak, fixed in c8492ef's last_expr_type per lit kind); m0_15 originally 28 → 16 after c8492ef); P0 + P1-2 + P1-3 + m0_04 净增 5 (P0 已 +3, 累计 baseline 自 11→14→18→19→20→**23**)). **25 个 FAIL 详情 + 分类**见 priority doc §8.1. Next: **M0 P1-4 剩余** (m0_19 recursion / m0_31 addr-of / m0_42 unsafe block) 或 **P1-1 CJK** (m0_50 高风险) — P1-3 done (m0_13/14/15 commits `afae9c9` + `c8492ef`) + P1-4 partial (m0_04 commits `5abb262` + `ef3352c`); 剩余 P1-4 (m0_19 / m0_31 / m0_42) + P1-1 (m0_50 CJK)。
